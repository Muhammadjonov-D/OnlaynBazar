...
public class UserService(IUnitOfWork unitOfWork, IMemoryCache memoryCache) : IUserService
{
    ...
    {
        var existUser = await unitOfWork.Users.SelectAsync(u => (u.PhoneNumber == user.PhoneNumber || u.Email == user.Email) && !u.IsDeleted);
        if (existUser is not null)
            throw new AlreadyExistException($"This user already exists with this phone={user.PhoneNumber}");
    ...
    public async ValueTask<User> UpdateAsync(long id, User user)
    {
        ...
        var alreadyExistUser = await unitOfWork.Users.SelectAsync(u => (u.PhoneNumber == user.PhoneNumber || u.Email == user.Email) && !u.IsDeleted && u.Id != id);
    ...
    public async ValueTask<(User user, string token)> LoginAsync(string phone, string password)
    {
        var existUser = await unitOfWork.Users.SelectAsync(
            expression: u => u.PhoneNumber == phone && !u.IsDeleted,
            includes: ["Role"])
    ...
    {
        var user = await unitOfWork.Users.SelectAsync(user => user.PhoneNumber == phone)
            ?? throw new NotFoundException($"User is not found with this phone={phone}");
    ...
    public async ValueTask<User> ChangePasswordAsync(string phone, string oldPassword, string newPassword)
    {
        var existUser = await unitOfWork.Users.SelectAsync(
            expression: u =>
                u.PhoneNumber == phone && PasswordHasher.Verify(oldPassword, u.Password) && !u.IsDeleted,
            includes: ["Role"])
            ?? throw new ArgumentIsNotValidException($"Phone or password is not valid");
